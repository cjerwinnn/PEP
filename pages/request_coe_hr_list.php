...
<div class="container-fluid mb-2">
    <div class="card shadow rounded-4">
        <div class="card-body p-4">
            <h5 class="text-muted">Certificate of Employment Request Approval (HR)</h5>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-sm small" id="ceTable" style="table-layout: fixed; width: 100%; font-size: 0.875rem;">
                    ...
                    <tbody>
                        <?php
                        $stmt = $conn->prepare("CALL COE_APPROVALLIST_HR()");
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo '<button class="btn btn-warning btn-sm ms-2 rounded-4" title="View COE Request" onclick="COE_HRView(\'' . addslashes($row['request_id']) . '\', \'' . addslashes($row['employee_id']) . '\', \'' . addslashes($row['coe_type']) . '\')">View</button>';
                            echo "</td>";
                            echo "</tr>";
                        }
