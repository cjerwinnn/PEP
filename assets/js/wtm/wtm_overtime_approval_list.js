...
function loadEmployees(dept, area) {
  ...
  fetch(`../fetch/wtm/wtm_overtime_employeelist.php?department=${encodeURIComponent(dept)}&area=${encodeURIComponent(area)}&month=${encodeURIComponent(month)}&year=${encodeURIComponent(year)}`)
    .then(response => {
    ...
    .then(data => {
      ...
      data.forEach(row => {
        ...
        <td class="text-start text-wrap" style="width:25%;">${row.employeename}</td>
...
function ViewOvertime(employeeId, overtimeId) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      window.location.href = 'wtm_approvalview_overtime.php';
    },
