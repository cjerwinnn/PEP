...
function loadOvertimeTypes(dropdownId = '#overtimetype_dropdown') {
    $.post("../fetch/wtm/wtm_overtime_types.php", {}, function (types) {
        const $dropdown = $(dropdownId);

        if (types.error) {
            $dropdown.html(`<option value="">Error loading types</option>`);
            return;
        }

        let options = `<option value="" disabled selected>Select a type...</option>`;
        types.forEach(t => {
            options += `<option value="${t.ottype} - ${t.description}">${t.ottype} - ${t.description}</option>`;
        });
